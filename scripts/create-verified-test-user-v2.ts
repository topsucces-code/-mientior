/**
 * Script to create a test user via the FIXED registration API
...
async function createVerifiedTestUser() {
  ...
  try {
    console.log('Creating test user via FIXED registration API...')
    
    // Step 1: Register the user via the API (now using Better Auth internally)
    ...
    if (!registerResponse.ok) {
      ...
      console.log('✅ User registered successfully with Better Auth')
    }
